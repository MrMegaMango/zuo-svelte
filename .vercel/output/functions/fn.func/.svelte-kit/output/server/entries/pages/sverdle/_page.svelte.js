import { e as ensure_array_like, h as head, f as bind_props, c as pop, p as push, d as stringify } from "../../../chunks/index2.js";
...
function _page($$payload, $$props) {
  ...
  $$payload.out += `<h1 class="visually-hidden svelte-1pg2j5l">Sverdle</h1> <form method="post"${attr("action", `${stringify(base)}/sverdle?/enter`)} class="svelte-1pg2j5l"><a class="how-to-play svelte-1pg2j5l"${attr("href", `${stringify(base)}/sverdle/how-to-play`)}>How to play</a> <div${attr("class", to_class("grid", "svelte-1pg2j5l", { "playing": !won, "bad-guess": form?.badGuess }))}><!--[-->`;
  for (let $$index_1 = 0, $$length = each_array.length; $$index_1 < $$length; $$index_1++) {
    let row = each_array[$$index_1];
    const current = row === i;
    const each_array_1 = ensure_array_like(Array.from(Array(5).keys()));
    $$payload.out += `<h2 class="visually-hidden svelte-1pg2j5l">Row ${escape_html(row + 1)}</h2> <div${attr("class", to_class("row", "svelte-1pg2j5l", { "current": current }))}><!--[-->`;
    for (let $$index = 0, $$length2 = each_array_1.length; $$index < $$length2; $$index++) {
      ...
      $$payload.out += `<div${attr("class", to_class("letter", "svelte-1pg2j5l", {
        "exact": exact,
        "close": close,
        "missing": missing,
        "selected": selected
  ...
  if (won || data.answers.length >= 6) {
    ...
    $$payload.out += `<div class="keyboard svelte-1pg2j5l"><button data-key="enter"${attr("disabled", !submittable, true)}${attr("class", to_class("", "svelte-1pg2j5l", { "selected": submittable }))}>enter</button> <button data-key="backspace" formaction="?/update" name="key" value="backspace" class="svelte-1pg2j5l">back</button> <!--[-->`;
    for (let $$index_3 = 0, $$length = each_array_2.length; $$index_3 < $$length; $$index_3++) {
      ...
      for (let $$index_2 = 0, $$length2 = each_array_3.length; $$index_2 < $$length2; $$index_2++) {
        ...
        $$payload.out += `<button${attr("data-key", letter)}${attr("class", to_class(clsx(classnames[letter]), "svelte-1pg2j5l"))}${attr("disabled", submittable, true)} formaction="?/update" name="key"${attr("value", letter)}${attr("aria-label", `${stringify(letter)} ${stringify(description[letter] || "")}`)}>${escape_html(letter)}</button>`;
      }
      $$payload.out += `<!--]--></div>`;
    }
    $$payload.out += `<!--]--></div>`;
